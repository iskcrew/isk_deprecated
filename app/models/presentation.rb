...
class Presentation < ActiveRecord::Base
  ...
  def slides
    Slide.joins(master_group: { groups: :presentation })
         .where(presentations: { id: self.id }).order("groups.position, slides.position")
  end
  ...
  def public_slides
    self.slides.where(slides: { public: true, deleted: false, replacement_id: nil })
  end
  ...
  def duration
    default_slides_time = self.delay * self.public_slides
